...
public class CeIISAInstructionLOADREF extends CeIISAInstruction {
   public CeIISAInstructionLOADREF(String mnemonic) {
      super(mnemonic);
   }
   ...
   protected void execute(ArrayList<Integer> params, CeIVMAPIMemory mem, CeIVMAPISpecialRegs regs, CeIVMAPIIOSubSys io) throws CeIVMMemoryException, CeIVMRuntimeException {
      int pointer = mem.read(regs.getSp());
      if (pointer == 0) {
         throw new CeIVMRuntimeException("Desreferenciameinto de puntero nulo en LOADREF.", regs.getPc());
      } else {
         mem.write(regs.getSp(), mem.read(pointer + (Integer)params.get(0)));
