...
public class CeIISAInstructionDEREF extends CeIISAInstruction {
   public CeIISAInstructionDEREF(String mnemonic) {
      super(mnemonic);
   }
   ...
   protected void execute(ArrayList<Integer> params, CeIVMAPIMemory mem, CeIVMAPISpecialRegs regs, CeIVMAPIIOSubSys io) throws CeIVMMemoryException, CeIVMRuntimeException {
      ...
      if (pointer == 0) {
         throw new CeIVMRuntimeException("Desreferenciameinto de puntero nulo en DEREF.", regs.getPc());
      } else {
         mem.write(regs.getSp(), mem.read(pointer));
